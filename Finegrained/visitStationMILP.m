...
% The optimization variables are - u1 u2 x1 x2 y1 y2 z1 z2 z3 z4 z5.
...
stationSize = [station(2)-station(1) station(4)-station(3)];
...
A_obs = zeros(nv*u_horizon, col);
b_obs = zeros(nv*u_horizon, 1);
...
% Inequality constraints to visit station.
A_sta = zeros(nx*u_horizon, col);
b_sta = zeros(nx*u_horizon, 1);
for i = 1:u_horizon
    A_sta(nx*i-3, nu*u_horizon+2+nx*(i-1)) = 1;
    A_sta(nx*i-2, nu*u_horizon+2+nx*(i-1)) = -1;
    A_sta(nx*i-3, nu*u_horizon+nx*x_horizon+(nz-1)*z_horizon+i) = M;
    A_sta(nx*i-2, nu*u_horizon+nx*x_horizon+(nz-1)*z_horizon+i) = M;
    
    A_sta(nx*i-1, nu*u_horizon+4+nx*(i-1)) = 1;
    A_sta(nx*i, nu*u_horizon+4+nx*(i-1)) = -1;
    A_sta(nx*i-1, nu*u_horizon+nx*x_horizon+(nz-1)*z_horizon+i) = M;
    A_sta(nx*i, nu*u_horizon+nx*x_horizon+(nz-1)*z_horizon+i) = M;
    
    b_sta(nx*i-3:nx*i, 1) = [station(2); -station(1); station(4); -station(3)];
end

A8 = zeros(1, col);
A8(1, nu*u_horizon+nx*x_horizon+(nz-1)*z_horizon + (1:z_horizon)) = ones(1, z_horizon);
b8 = u_horizon-1;



A = [A_eq; A_obs; A_z; A_fin; A_init; A_sta; A8];
b = [b_eq; b_obs; b_z; b_fin; b_init; b_sta; b8];
