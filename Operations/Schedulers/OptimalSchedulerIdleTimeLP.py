...
schedule.to_csv(f"Plans/idleTimeMinInitialLP.csv")
draw_tree_schedule(schedule, "Images/idleTimeMinInitialLP.png")
...
# Create optimization variables, and define the constraints.
X, Y, Z, S, C, S_job, C_job, C_max = create_opt_variables(idleSolver, job_list, all_machines, Mj)
define_constraints(idleSolver, X, Y, Z, S, C, S_job, C_job, C_max, job_list, parent_ids, Mj)
# add_no_c_inflation(idleSolver, S, C, job_list, Mj)

# Define objective to minimize the idle times.
optimum = solver.Objective().Value()
create_idle_time_objective(idleSolver, S, C, C_max, job_list, parent_ids, Mj, optimum)
...
display_solution_stats_lp(idleSolver, status, horizon, solutionEnd-solutionStart)
...
# Save the schedule and draw it.
schedule.to_csv(f"Plans/idleTimeMinFinalLP.csv")
draw_tree_schedule(schedule, "Images/idleTimeMinFinalLP.png")