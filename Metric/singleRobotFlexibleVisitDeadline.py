...
for k in range(planHorizon-1):
    ...
    for x in range(numStates):
        ...
        o.add( Implies(X[k] == int(x), Or(cons)) )

# Visit workstation at visit_1 at least once.
cons = [X[k] == int(get_neighbors(visit_1, grid, obs, stations)[0]) for k in range(visit_1_deadline)]
o.add(Or(cons))

# Visit workstation at visit_2 at least once.
cons = [X[k] == int(get_neighbors(visit_2, grid, obs, stations)[0]) for k in range(visit_1_deadline, planHorizon)]
o.add(Or(cons))
