...
# Position and occupied variables for the robots.
numRobots = 4
...
initialPositions = [120, 119, 23, 25]
finalPositions = [21, 22, 23, 20]
...
n_robots_visit_station_for_duration(solver, X, Oc, grid, obs, stations,
                0, planHorizon, numVisitors, RF_Welder, visit_1_duration)

# Visit Grommet for 10 timesteps.
numVisitors = 1
n_robots_visit_station_for_duration(solver, X, Oc, grid, obs, stations,
                0, planHorizon, numVisitors, Grommet, visit_1_duration)

# Visit Mega_Stitch for 10 then Long_Arm for 5.
numVisitors = 1
n_robots_sequence_two_visits(solver, X, Oc, grid, obs, stations,
                0, planHorizon, numVisitors, Mega_Stitch, Long_Arm,
                visit_1_duration, visit_2_duration)

# # Robots visit Mega_Stitch for 10 Machine 3 for 10, and RF_Welder for 5.
# numVisitors = 3
# n_robots_sequence_three_visits(solver, X, Oc, grid, obs, stations,
#     0, planHorizon, numVisitors, Mega_Stitch, Sewing_Machine_3, RF_Welder,
#     10, 10, 5
# )
...
df = pd.DataFrame()
df[['Path1','Path2','Path3','Path4']] = path.T
df[['Occupied1','Occupied2','Occupied3','Occupied4']] = occupied.T
print(df)

animate_path(numRobots, planHorizon, path, grid, stations, obs, bounds, spaceStep, "Videos/modularMulti.mp4")