...
def create_opt_variables(model: cp_model.CpModel, job_list: list, machine_ids: list, grouped_machine_ids: list):
    ...
    Args:
        ...
        job_list: List of lists, where each sub-list is a job made up of
                    task dictionaries.
        machine_ids: List of unique identifiers for each machine.
        grouped_machine_ids: List of lists, where each sub-list contains the unique identifiers
...
def define_constraints(model: cp_model.CpModel, X, Y, Z, S, C, S_job, C_job, C_max, job_list, parent_ids, grouped_machine_ids):
    ...
    for job_idx, job in enumerate(job_list):
        for task_idx, task in enumerate(job):
            ...
            model.AddExactlyOne(
                X[job_idx, task_idx, machine] for machine in grouped_machine_ids[task["machine_type"]]
            )
...
def create_idle_time_objective(model: cp_model.CpModel, S, C, C_max, job_list, parent_ids, grouped_machine_ids, optimum):
